...
\begin{frame}{研究局限与后续方向}
  \begin{itemize}
    \item \textbf{语言范围}：当前最小配对分析聚焦中文；英文平行语料仍在整理，因此这里的结论暂时只针对中文场景。
    \item \textbf{ICC 绝对值}：9 个单元的 ICC 都落在 Koo \& Li (2016) 所定义的 poor 区间；这更像是在说明人机题项级对应本来就比较弱，而不是实验"没做出来"。
    \item \textbf{对齐偏斜的直接检验}：如果后续用女性评分者数据做再校准，并观察 ICC 是否系统上升，就能更直接检验"对齐偏斜"这一路径。
    \item \textbf{模型生态}：本研究覆盖了主流的中、西方通用 LLM；更小参数模型或专用审核模型是否呈现同样模式，还需要继续检验。
    \item \textbf{维度细粒度}：目前较稳健的结论主要集中在一级维度；二级 / 三级维度之间更细的交互关系，还有待展开。
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusions}
  \footnotesize
  \begin{enumerate}
    \item \textbf{中英差距最值得关注的是中文端更大的离散度，而不只是平均分差。}方差比达到 $12.7$--$15.0$ 倍，更像是在反映各模型对中文语料掌握程度不同，而不只是任务定义不同。
    \item \textbf{9 个 LLM 和人类都更倾向于把"女人版"评得更高，且共同集中在\emph{性化 + 外貌}内容上。}这说明人机在偏差方向和热点维度上确实有稳定重合。
    \item \textbf{评分者是谁，比量表形式更能影响结果。}女性被试的总体效应量约为男性的 3 倍，组间差也约为组内差的 4--8 倍，说明被试性别才是更强的异质来源。
    \item \textbf{并不存在一个普适的"最像人"模型。}top-1 会随着被试子群和尺度切换，而且无论人类还是 LLM，题项级结果都会随尺度明显重排，所以任何"最像人"的结论都必须说明是\emph{像哪一群人、在哪一种尺度}。
  \end{enumerate}
