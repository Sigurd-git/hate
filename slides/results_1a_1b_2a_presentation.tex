...
% Prefer PNG companions for external matplotlib figures when available.
% This avoids PDF font embedding issues that can surface as garbled CJK text
% in some viewers while keeping native vector graphics for the slide chrome.
\ExplSyntaxOn
\cs_new_eq:NN \OriginalIncludegraphics \includegraphics
\RenewDocumentCommand{\includegraphics}{O{} m}{%
  \tl_set:Nx \l_tmpa_tl { #2 }%
  \regex_replace_once:nnN { \.pdf \z } { .png } \l_tmpa_tl
  \file_if_exist:nTF { \l_tmpa_tl }
    { \OriginalIncludegraphics[#1]{\l_tmpa_tl} }
...
\begin{frame}{报告结构}
  \begin{enumerate}
    ...
    \item 研究 2a$\cdot$CS（Cross-Scale similarity，跨尺度相似度）：与 H1b.3 对位的人机跨尺度相似性（H2a.CS.1--.2）
    \item 研究 2a$\cdot$ICC（Intraclass Correlation Coefficient，类内相关系数）：同尺度内的人机题项级相似度（H2a.ICC.1--.3）
    \item 综合结论与研究局限
  \end{enumerate}
\end{frame}
...
\begin{frame}{研究 1a：研究问题}
  \begin{block}{核心问题：检验目前LLM对仇恨识别的任务能力}
  ...
  \begin{itemize}
    ...
    \item 指标：F1（分类准确度）+ Brier（将0-5的打分除以5，以代表仇恨概率，再算离label的标准差，越小代表越准确，叫概率校准度）。
...
\begin{frame}{研究 1a：两个关键发现}
  \begin{alertblock}{发现一：中英文F1均值差距不大，英文略好}
    \small $\Delta_{F1}(zh-en)$：zero-shot $M=-0.046$（SD $0.069$，p不显著）；CoT $M=-0.050$（SD $0.063$，不显著）。$\Delta_{\mathrm{Brier}}(zh-en)$：约 $+0.04$（不显著）。均值差距不大，H1a.1 与 H1a.2 都能大致解释。
  \end{alertblock}

  \begin{alertblock}{发现二：中文端模型之间差异大得多——关键差异}
...
\begin{frame}{研究 1a：F1 榜首不一定是最佳——Llama 分类好却过度自信，（F1最高但概率校准低）Qwen 概率校准最好}
  \footnotesize
  \begin{table}
    \centering
  ...
  \vspace{0.4em}
  \begin{block}{关键观察}
    \small 在英文 zero-shot 设置下，Llama-4-Maverick 的 F1 最高（0.698），但其 Brier 分数也更高（0.297），说明其概率输出校准较差、存在过度自信；Qwen-2.5-72B 的 F1 略低（0.686），但 Brier 最低（0.254），说明其预测概率更接近真实结果频率。\textbf{F1 和 Brier 可以相互独立。}
  \end{block}
\end{frame}
...
\begin{frame}{研究 1b：27 单元完整统计（1/2）——所有模型三尺度均显著 $\Delta_{F-M}>0$}
  ...
  \begin{table}
    ...
    \begin{tabular}{llccccc}
      \toprule
      模型 & 尺度 & $\Delta_{F-M}$ & 95\% CI & $d_z$ & 女\,:\,平\,:\,男 & $q$ (FDR的p) \\
      \midrule
...
\begin{frame}{研究 1b：持平带随尺度分辨率从 $80\%$ 收缩到 $38\%$——随打分尺度更细不公平被更突出地记录}
  \centering
  \includegraphics[width=\textwidth,height=0.66\textheight,keepaspectratio]{\artifacts/directionality_three_scale_grid.pdf}

  \scriptsize 红 = 女人版更高；灰 = 持平；蓝 = 男人版更高。\textbf{持平的比例随尺度粒度变细而下降}：9 模型中位 $80.1\% \to 62.0\% \to 37.7\%$（3 点 $\to$ 7 点 $\to$ 滑块）——不是偏差减少了，而是更细的尺度把原本"持平"的小差异也记录下来了。
\end{frame}
...
\begin{frame}{研究 1b：固定同一个模型看，三尺度下的 10 维 $d_z$ 向量有多像自己？}
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \begin{table}
    \centering
    \begin{tabular}{lccc}
      \toprule
      模型 & 3pt vs 7pt & 7pt vs slider & 3pt vs slider \\
      \midrule
      Claude Opus 4.5 & $-0.27$ & $+0.62$ & $-0.20$ \\
      GPT-5.1 & $+0.73$ & $+0.65$ & $+0.62$ \\
      Gemma 4 31B & $+0.80$ & $+0.03$ & $-0.05$ \\
      Llama Maverick & $+0.43$ & $+0.55$ & $+0.48$ \\
      Qwen 2.5 72B & $+0.35$ & $+0.42$ & $+0.68$ \\
      GLM 4.6 & $+0.19$ & $+0.10$ & $+0.01$ \\
      Kimi K2 & $+0.48$ & $+0.47$ & $+0.56$ \\
      DeepSeek R1 & $+0.10$ & $-0.41$ & $+0.67$ \\
      DeepSeek V3.2 & $-0.43$ & $+0.08$ & $-0.09$ \\
      \bottomrule
    \end{tabular}
  \end{table}
  ...
  \small
  \begin{itemize}
    \item 每一行比较一个模型在三对尺度上的\textbf{10 维一级领域 $d_z$ 向量} Spearman $\rho$。
    \item GPT-5.1、Kimi、Llama、Qwen 跨尺度相对稳定；Claude、DeepSeek V3.2、GLM 4.6 明显不稳；Gemma 与 DeepSeek R1 只在部分尺度对上相似。
  \end{itemize}
\end{frame}

\begin{frame}{研究 1b：H1b.2\\9 个模型均把"性化 + 外貌"列打得最深——维度特征规律}
  \centering
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}{研究 1b：持平不是中立——偏差被集中到少数决策分化的题项上}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \includegraphics[width=\linewidth,height=0.24\textheight,keepaspectratio]{\oneb/attack_7pt_likert/panels/fig1_paired_scores_panel_2.pdf}\\[-0.2em]
      {\scriptsize Claude Opus 4.5（持平 65.0\%，不持平时 64:1）}
    \column{0.48\textwidth}
      \includegraphics[width=\linewidth,height=0.24\textheight,keepaspectratio]{\oneb/attack_7pt_likert/panels/fig1_paired_scores_panel_4.pdf}\\[-0.2em]
      {\scriptsize Llama Maverick（持平 71.4\%，不持平时 9.6:1）}
  \end{columns}
  \vspace{0.6em}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
      \includegraphics[width=\linewidth,height=0.24\textheight,keepaspectratio]{\oneb/attack_7pt_likert/panels/fig1_paired_scores_panel_8.pdf}\\[-0.2em]
      {\scriptsize Qwen 2.5 72B（持平 73.9\%，不持平时 96:1）}
    \column{0.48\textwidth}
      \includegraphics[width=\linewidth,height=0.24\textheight,keepaspectratio]{\oneb/attack_7pt_likert/panels/fig1_paired_scores_panel_1.pdf}\\[-0.2em]
...
\begin{frame}{研究 2a：研究问题与设计}
  ...
  \begin{itemize}
    \item \textbf{被试规模}：779 个有效评分 session。
...
\begin{frame}{研究 2a：被试信息（session $\times$ 性别 $\times$ 尺度）}
  ...
  \vspace{0.3em}
  \small \textbf{独立被试人数} $=712$；其中 645 人只留下 1 个 session、67 人留下 2 个 session。合计 $645\times 1 + 67\times 2 = 779$ session。\\
  性别在总体上几乎平衡（女 50.3\% / 男 49.7\%）；各尺度条件内也近似平衡（女性占比 49\%--53\%）。\\
  全文后续 2a 所有 $d_z$、ICC、$\rho$ 均以 779 个 \emph{session} 为分析单元。
\end{frame}

\begin{frame}{研究 2a：被试人口学特征}
  \tiny
  \setlength{\tabcolsep}{3pt}
...
{\setbeamercolor{frametitle}{bg=black!90,fg=white}
\begin{frame}{研究 2a：四条假设（与 1b 配对）}
  \footnotesize
  ...
  \begin{block}{H2a.3 被试性别差异假设（2a 特有）}
    \small 女性被试和男性被试 $d_z$ 存在差异（日常经验影响敏感度），两组方向均为 $\Delta_{F-M}>0$。
  \end{block}
...
\begin{frame}{研究 2a：2$\times$3 重复测量 ANOVA 显示被试性别主效应更强}
  \small
  \begin{itemize}
    \item 以 371 个题项为重复测量单元；将各格子的 $\Delta_{F-M}$ 按各自量表上限归一化后，检验\textbf{被试性别（男 / 女）$\times$量表（3 点 / 7 点 / 滑块）}。
    \item \textbf{被试性别主效应显著}：$F(1,370)=111.17$，$p<.001$，$\eta_G^2=.044$。
    \item \textbf{量表主效应也显著，但更小}：$F(2,740)=9.44$，$p_{GG}<.001$，$\eta_G^2=.007$。
    \item \textbf{交互不显著}：$F(2,740)=1.79$，$p_{GG}=.169$。
  \end{itemize}

  \vspace{0.3em}
  \begin{alertblock}{判决}
    \small 量表形式并非完全无关，但其解释量明显小于被试性别（$\eta_G^2$: $.007$ vs $.044$）；\textbf{H2a.4 支持}。
  \end{alertblock}
\end{frame}

\begin{frame}{研究 2a：人类被试中少有题项平均分完全一致，但女版得分明显高于男版}
  \centering
  \begin{columns}[T,onlytextwidth]
    \column{0.33\textwidth}
  ...
  \vspace{0.1em}
  \small 这张图复刻了前面 LLM 版本的连接散点图：每条线段连接同一题目的男版与女版平均分。横轴 = 男人版题项平均分；纵轴 = 女人版题项平均分。\emph{人类被试中完全持平的题项明显更少}，说明人类评分不像 LLM 那样规整；但即便如此，三种尺度下仍都能清楚看到大量点和线段位于对角线上方，即\textbf{女版攻击性评分明显高于男版}。
\end{frame}
...
\begin{frame}{研究 2a 小结：四条假设全部支持——人机同方向，被试性别是最稳健异质来源}
  ...
  \begin{table}
    ...
    \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{0.22\textwidth}>{\RaggedRight\arraybackslash}p{0.28\textwidth}X}
      ...
      H2a.4 尺度 $\ll$ 性别 & 性别主效应 $>$ 量表主效应 & \textbf{支持}（RM-ANOVA：性别效应量更大，交互不显著） \\
      \bottomrule
    \end{tabularx}
  \end{table}
...
% ===========================================================================
\section{研究 2a$\cdot$CS（Cross-Scale similarity，跨尺度相似度）：与 1b 的 H1b.3 对位}

\begin{frame}{研究 2a$\cdot$CS：研究问题与设计}
  \small\emph{从 1b 的 H1b.3 过渡到 2a}：在模型端，我们已经看到"三种尺度是不是在测同一件事"必须按层级分开说；因此到了 2a，最自然的下一步不是先问"哪个模型最像人"，而是先问\textbf{人类自己在三尺度下稳不稳}。
  ...
  \begin{itemize}
    \item \textbf{与 1b 的对位关系}：H1b.3 问的是模型端跨尺度相似性；本节的 H2a.CS.1 / .2 则把同一个问题带回人类，并直接放在人机比较框架里。
...
{\setbeamercolor{frametitle}{bg=black!90,fg=white}
\begin{frame}{研究 2a·CS：两条假设（与 H1b.3 对位）}
  \footnotesize
...
\begin{frame}{研究 2a·CS：人类三个被试群体在三对尺度上的 $\rho$ 都很低}
  \footnotesize
  \setlength{\tabcolsep}{6pt}
  \begin{table}
    \centering
    \begin{tabular}{lccc}
      \toprule
      被试子群 & 3pt vs 7pt & 7pt vs slider & 3pt vs slider \\
      \midrule
      全体被试 & $+0.118$ & $+0.091$ & $+0.124$ \\
      男性被试 & $+0.105$ & $+0.008$ & $+0.066$ \\
      女性被试 & $+0.011$ & $+0.077$ & $+0.108$ \\
      \bottomrule
    \end{tabular}
  \end{table}
  ...
  \small
  \begin{itemize}
    \item 9 个单元的 $\rho$ \textbf{全部低于 0.13}；多数 CI 跨越 $0$。
    \item 也就是说，对人类而言，同一批题一旦换尺度，\emph{哪一道题更偏}的排序就会明显重组。
  \end{itemize}
\end{frame}
...
% ===========================================================================
\section{研究 2a$\cdot$ICC（Intraclass Correlation Coefficient，类内相关系数）：同尺度内的人机题项级相似度}

\begin{frame}{研究 2a$\cdot$ICC：研究问题与设计}
  \small\emph{从 2a$\cdot$CS 到 2a$\cdot$ICC 的过渡}：既然上一节已经看到，无论人类还是 LLM，题项级结果都会随着尺度明显重排，那么"哪个模型最像人"这个问题就必须\textbf{固定在同一尺度内}来问。本节就在这个前提下，比的是\emph{同尺度内}的人机题项级偏差模式。

  \begin{block}{核心问题}
    \small 在固定同一评分尺度时，哪一个 LLM 的偏差分布与人类最接近？哪一句被判为最偏差这一问题上人机是否达成共识？
  \end{block}

  \vspace{0.8em}
  \small \textbf{题项级偏差向量}：$\Delta_i^\text{人} = \bar{y}_{i,\text{女}}^\text{人} - \bar{y}_{i,\text{男}}^\text{人}$；$\Delta_i^\text{模} = y_{i,\text{女}}^\text{模} - y_{i,\text{男}}^\text{模}$（\textbf{同尺度内比较}）。\\[0.5em]
  \small \textbf{主指标}：ICC(2,1) 绝对一致；对照：ICC(3,1) 一致性。Bootstrap $B = 2000$ 95\% CI。\\[0.5em]
  \small \textbf{9 个单元}：3 被试子群（全体 / 男 / 女）$\times$ 3 尺度；每单元遍历 9 个 LLM、选 ICC(2,1) 最高者。
\end{frame}
...
\begin{frame}{全文假设清单（1/2）}
  ...
  \begin{table}
    ...
    \begin{tabular}{lll}
      ...
      H2a.4 尺度 $\ll$ 性别 & 性别主效应 $>$ 量表主效应 & \textbf{支持}（RM-ANOVA：$\eta_G^2=.044$ $> .007$） \\
      \bottomrule
    \end{tabular}
  \end{table}
\end{frame}
...
\begin{frame}{研究局限与后续方向}
  \begin{itemize}
    \item \textbf{语言范围}：目前的最小配对分析主要做在中文上，英文平行语料我还在继续整理，所以现在能给出的结论暂时只限定在中文场景里。
    \item \textbf{ICC 绝对值}：9 个单元的 ICC 都落在 Koo \& Li (2016) 的 poor 区间；我倾向于把它看作人机题项级对应本来就比较弱的真实反映。
    \item \textbf{对齐偏斜的直接检验}：后续如果能用女性评分者的数据做再校准，再观察 ICC 会不会系统地抬升。
    \item \textbf{模型生态}：本研究覆盖的主要是主流的中、西方通用 LLM；至于更小参数的模型或者专用的审核模型会不会呈现同样的模式，还需要在之后的工作里继续检验。
    \item \textbf{维度细粒度}：目前相对稳健的结论主要还集中在一级维度，二级 / 三级维度之间更细的交互关系，就留给后续再展开。
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusions}
  \footnotesize
  \begin{enumerate}
    \item \textbf{中英差距里最值得关注的，其实是中文端更大的离散度，而不只是平均分差。}方差比能到 $12.7$--$15.0$ 倍，我更倾向于把它读成各模型对中文语料掌握程度的差异，而不只是任务定义层面的分歧。
    \item \textbf{9 个 LLM 和人类都倾向于把"女人版"评得更高，而且共同集中在\emph{性化 + 外貌}这块内容上。}这让我觉得，人机之间在偏差方向和热点维度上确实存在比较稳定的重合。
    \item \textbf{比起量表形式，评分者本身是谁反而更能左右结果。}女性被试的总体效应量大约是男性的 3 倍；在按量表归一化后的题项偏差上，2$\times$3 RM-ANOVA 也显示性别主效应（$\eta_G^2=.044$）明显大于量表主效应（$.007$）。
    \item \textbf{至少在目前的数据里，并不存在一个普适意义上"最像人"的模型。}top-1 会随着被试子群和尺度切换，而且不管是人类还是 LLM，题项级结果都会随尺度明显重排；所以任何"最像人"的说法，都得交代清楚是\emph{像哪一群人、在哪一种尺度下。}
  \end{enumerate}
