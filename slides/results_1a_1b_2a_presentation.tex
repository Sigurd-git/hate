...
\begin{frame}{研究 2a$\cdot$ICC：研究问题与设计}
  \begin{block}{核心问题}
    \small 人机在方向上已达成一致；在题项层面，哪一个 LLM 的偏差分布与人类最接近？哪一句被判为最偏差这一问题上人机是否达成共识？
  \end{block}

  \vspace{1.0em}
  \small \textbf{题项级偏差向量}：$\Delta_i^\text{人} = \bar{y}_{i,\text{女}}^\text{人} - \bar{y}_{i,\text{男}}^\text{人}$；$\Delta_i^\text{模} = y_{i,\text{女}}^\text{模} - y_{i,\text{男}}^\text{模}$（同尺度内比较）。\\[0.5em]
