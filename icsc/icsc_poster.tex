...
Online gendered hostility against women is a visible form of harm in Chinese digital spaces, where misogynistic language and anti-feminist discourse are common on major platforms~[1,2]. Large language models (LLMs) are increasingly used to rate, flag, and code hostile content, but it remains unclear whether their ratings track human perceptions when the target of the same attack is changed from a man to a woman~[3,4]. Existing evidence is limited in two ways. First, most human--LLM comparisons come from English-language corpora, whereas Chinese gendered attacks use different vocabularies, gender references, and platform-specific expressions~[1]. Second, naturally occurring corpora confound target gender with content extremity and context, so raw female-versus-male contrasts cannot isolate the effect of target gender~[7]. Even after this contrast is controlled, agreement is not a single number: it can depend on the rating scale and on which human reference group is used~[5,6]. We therefore use manually constructed Chinese gender-reversal minimal pairs to ask whether human and LLM ratings agree in the direction, magnitude, and content structure of the female-minus-male attack-rating gap when the attack content is held fixed.
...
Human--LLM similarity in perceiving gendered attacks is conditional: they share the direction and broad content structure of the effect, but diverge once the comparison asks how large the effect is, which human group defines the reference, which scale elicits the judgement, and which individual sentences carry it. This pattern suggests that LLMs can reproduce a coarse social signal without reproducing the finer perceptual structure through which humans assign harm, severity, and salience to language. As LLMs increasingly mediate social judgement, the question is therefore not whether machines ``match humans'' in general, but whose perception is being approximated, at what level of granularity, and for what institutional purpose.
