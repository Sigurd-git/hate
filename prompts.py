...
Supported paradigms: ``zero_shot`` and ``chain_of_thought``.
"""
...
PromptParadigm = Literal["zero_shot", "chain_of_thought"]
...
def render_prompt(language: str, paradigm: PromptParadigm, text: str) -> str:
    ...
    ----------
    language:
        Two-letter key supported by ``PROMPTS`` (currently ``zh`` or ``en``).
    paradigm:
        Must be one of ``zero_shot`` or ``chain_of_thought``.
    text:
        The sentence that should be embedded into the template.
    """
